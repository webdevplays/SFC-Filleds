...
export default function GroupsPage({ currentAdminId }: GroupsPageProps) {
  ...
  const handleRetireGroup = (grp: Group) => {
    setDeleteDialog({
      isOpen: true,
      title: 'Retire Field Group',
      description: `Are you sure you want to retire group ${grp.GroupName}? This will soft-delete their administrative status.`,
      criticalWarning: 'The group status will be changed to Retired.',
      onConfirm: async () => {
        try {
  ...
  const handleHardDeleteGroup = (grp: Group) => {
    setDeleteDialog({
      isOpen: true,
      title: 'PERMANENTLY Erase Field Group',
      description: `Are you sure you want to PERMANENTLY delete group "${grp.GroupName}"? All survey information associated with this group will remain, but this group itself will be completely erased from the system selection. This action is irreversible.`,
      criticalWarning: 'This action is PERMANENT and cannot be undone!',
      onConfirm: async () => {
        try {
